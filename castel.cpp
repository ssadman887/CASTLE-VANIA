...
int c5 = 0, l1 = 0, f1 = 0, c = 100, r1 = 0, scar = 0, scar1 = 0, life = 5, q4 = 2, ry7 = 0, bal = 0;
point a[5], ebul[10], ebul1[10];
float p = .50;
map<int, int> mbc, mec, mebul, mebul1, zik, pik;
    ...
    for (int i = 0; i < 10; i++)
    {
        if (mbc[i] == 1)
        {
            ...
            else
            {
                for (int j = 0; j < 5; j++)
                {
                    if (bul_x[i] >= a[j].x - 10 && bul_x[i] <= a[j].x + 50 && bul_y[i] <= a[j].y + 40 && bul_y[i] >= a[j].y - 15)
                    {
                        bul_x[i] = 1500;
                        a[j].x = 1500;
                        scar = scar + 5;
...
void emove(int i1, int x, int y)
{
    ...
    if (mec[i1] == 1)
    {
        ...
        if (ry7 == 1)
        {
            a[i1].x = a[i1].x + pik[i1];
        }
        if (zik[i1] == 100 || a[i1].x <= 0 || a[i1].x >= (1200 - 50))
        {
            pik[i1] = -1 * pik[i1];
            zik[i1] = 0;
        }
...
int main(int agr, char *args[])
{
    ...
    SDL_FreeSurface(surface);
    SDL_Rect bac, alu1, alu2, alu3, bullet, e1, e2, hp, qu;
    bac.h = 800;
    bac.w = 1200;
    bac.x = 0;
    bac.y = 0;
    ...
    SDL_FreeSurface(surface);
    surface = IMG_Load("midia/PlayButton.png");
    ...
    SDL_FreeSurface(surface);
    surface = IMG_Load("midia/level2.png");
    SDL_Texture *lev2 = SDL_CreateTextureFromSurface(rend, surface);
    SDL_FreeSurface(surface);
    surface = IMG_Load("midia/HelpButton.png");
    SDL_Texture *HELP = SDL_CreateTextureFromSurface(rend, surface);
    SDL_FreeSurface(surface);
    surface = IMG_Load("midia/QuitButton.png");
    SDL_Texture *QUITE = SDL_CreateTextureFromSurface(rend, surface);
    SDL_FreeSurface(surface);
    surface = IMG_Load("midia/cd.png");
    SDL_Texture *CD = SDL_CreateTextureFromSurface(rend, surface);
    SDL_FreeSurface(surface);
    surface = IMG_Load("midia/return.png");
    SDL_Texture *RU = SDL_CreateTextureFromSurface(rend, surface);
    SDL_FreeSurface(surface);
    surface = IMG_Load("midia/ins.png");
    SDL_Texture *INS = SDL_CreateTextureFromSurface(rend, surface);
    SDL_FreeSurface(surface);
    surface = IMG_Load("midia/credits.png");
    SDL_Texture *CDS = SDL_CreateTextureFromSurface(rend, surface);
    SDL_FreeSurface(surface);
    ...
    while (q4 != 1)
    {
        ...
        else if (q4 == 22)
        {
            ...
            for (int i = 0; i < 50; i++)
            {
                SDL_RenderClear(rend);
                SDL_RenderCopy(rend, back, NULL, &bac);
                SDL_RenderCopy(rend, lev2, NULL, &lev);
                SDL_RenderPresent(rend);
                lev.h = lev.h + 10;
                lev.w = lev.w + 10;
                lev.x = lev.x - 5;
        ...
        else if (q4 == 2)
        {
            ...
            {
                // printf("%d %d\n",mousex,mousey);
                if (mousex >= hp.x && mousex <= (hp.x + hp.w) && mousey >= hp.y && mousey <= (hp.y + hp.h))
                {
                    q4 = 23;
                }
            }
            ...
            {
                // printf("%d %d\n",mousex,mousey);
                if (mousex >= qu.x && mousex <= (qu.x + qu.w) && mousey >= qu.y && mousey <= (qu.y + qu.h))
                {
                    q4 = 1;
                }
            }
        ...
        else if (q4 == 23)
        {
            ...
            SDL_RenderClear(rend);
            SDL_RenderCopy(rend, back, NULL, &bac);
            SDL_RenderCopy(rend, RU, NULL, &ru);
            ...
            {
                // printf("%d %d\n",mousex,mousey);
                if (mousex >= ru.x && mousex <= (ru.x + ru.w) && mousey >= ru.y && mousey <= (ru.y + ru.h))
                {
                    q4 = 2;
                }
            }
        ...
        else if (q4 == 24)
        {
            ...
            SDL_RenderCopy(rend, back, NULL, &bac);
            SDL_RenderCopy(rend, RU, NULL, &ru);
            SDL_RenderCopy(rend, CDS, NULL, &ins);
            SDL_RenderPresent(rend);
            SDL_Delay(1000 / 60);
            int buttons = SDL_GetMouseState(&mousex, &mousey);
            if (buttons & SDL_BUTTON(SDL_BUTTON_LEFT))
